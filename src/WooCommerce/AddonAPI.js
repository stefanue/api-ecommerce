...
function AddonAPI(opt) {
    if (!(this instanceof AddonAPI)) {
        return new AddonAPI(opt);
    }
...
AddonAPI.prototype._setDefaultsOptions = function(opt) {
    this.url = opt.url;
...
AddonAPI.prototype._normalizeQueryString = function(url) {
...
AddonAPI.prototype._getUrl = function(endpoint) {
    let url = this.url.slice(-1) === "/" ? this.url : `${this.url}/`;
    const api = this.wpAPI ? `${this.wpAPIPrefix}/` : "wp-json/wc-product-add-ons/";
...
AddonAPI.prototype._getOAuth = function() {
    const data = {
        consumer: {
...
AddonAPI.prototype.join = function(obj, separator) {
    const arr = [];
...
AddonAPI.prototype._request = async function(method, endpoint, newData) {
...
AddonAPI.prototype.get = async function(endpoint, data, callback) {
    return await this._request("GET", endpoint, data, callback);
};

AddonAPI.prototype.post = async function(endpoint, data, callback) {
    return await this._request("POST", endpoint, data, callback);
};

AddonAPI.prototype.put = async function(endpoint, data, callback) {
    return await this._request("PUT", endpoint, data, callback);
};

AddonAPI.prototype.delete = async function(endpoint, callback) {
    return await this._request("DELETE", endpoint, null, callback);
};

AddonAPI.prototype.options = async function(endpoint, callback) {
